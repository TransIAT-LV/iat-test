...
dir.create(raw.directory, showWarnings = FALSE, recursive = TRUE)
...
raw.iat <- as.data.frame(t(data_list[['transiat']]))
```
...
raw.iat$blockName <- ifelse(raw.iat$data.condition %in% c('transWords/Bad words,cisWords/Good words'), 
                        ifelse(raw.iat$data.block %in% c(3,6), 'B3', 
                               ifelse(raw.iat$data.block %in% c(4,7), 'B4', NA)), 
                        ifelse(raw.iat$data.condition %in% c('cisWords/Bad words,transWords/Good words'), 
...
blockOrder$iat.order <- ifelse(blockOrder$block3Cond %in% c('transWords/Bad words,cisWords/Good words'), 
                        'compatibleFirst', 
                        ifelse(blockOrder$block3Cond %in% c('cisWords/Bad words,transWords/Good words'), 
